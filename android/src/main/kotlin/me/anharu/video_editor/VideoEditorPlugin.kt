...
public class  VideoEditorPlugin: FlutterPlugin, MethodCallHandler, PluginRegistry.RequestPermissionsResultListener, ActivityAware {
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    if (call.method == "getPlatformVersion") {
      ...
      val fileName: String = call.argument("name")?: run {
        result.error("file_name_not_found","the file name is not found.",null)
        return
      }
      ...
      val path: String = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES).getAbsolutePath();
      println(path)
      val generator = VideoGeneratorService(Mp4Composer(fileName, path +  File.separator +"this.mp4"))
